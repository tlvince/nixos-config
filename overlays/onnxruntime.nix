final: prev: let
  lib = final.lib;
  cpuinfoLoggingPatch = final.writeText "onnxruntime-cpuinfo-logging.patch" ''
  ...
  '';
  dropAarch64BadPlatform = pkg:
    pkg.overridePythonAttrs (
      old: let
        meta = old.meta or {};
        remaining = builtins.filter (platform: platform != "aarch64-linux") (meta.badPlatforms or []);
        updatedMeta =
          (lib.removeAttrs meta ["badPlatforms"])
          // lib.optionalAttrs (remaining != []) {
            badPlatforms = remaining;
          };
      in {
        meta = updatedMeta;
      }
  ...
  pythonPackagesExtensions =
    ...
    ++ [
      (pythonFinal: pythonPrev: {
        rapidocr-onnxruntime = dropAarch64BadPlatform pythonPrev.rapidocr-onnxruntime;
        rapidocr = dropAarch64BadPlatform pythonPrev.rapidocr;
      })
    ];
}
